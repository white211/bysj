...
import cn.white.bysj.admin.dao.IRoleDao;
...
public class RoleServiceImpl extends BaseServiceImpl<Role, Integer> implements IRoleService {
	...
	public void delete(Integer id) {
		Role role = find(id);
		Assert.state(!"SuperAdministrator".equals(role.getRoleKey()),"超级管理员角色不能删除");
		super.delete(id);
	}
	...
	public void grant(Integer id, String[] resourceIds) {
		...
		Assert.state(!"SuperAdministrator".equals(role.getRoleKey()),"超级管理员角色不能进行资源分配");
		Resource resource;
